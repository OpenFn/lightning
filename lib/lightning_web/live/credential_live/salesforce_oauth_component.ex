defmodule LightningWeb.CredentialLive.SalesforceOauthComponent do
    ...
    <%= render_slot(
      ...
      {Phoenix.LiveView.TagEngine.component(
         ...
         [
           ...
           id: "salesforce-oauth-inner-form-#{@id}"
         ],
  ...
  def render(%{client: nil} = assigns) do
    ...
    <div id={@id} class="flex items-center place-content-center pt-2">
      <div class="text-center">
        ...
        <span class="text-sm">
          Salesforce authorization has not been set up on this instance.
        </span>
      </div>
    </div>
    """
  ...
  def error_block(%{type: :userinfo_failed} = assigns) do
    ...
    <div class="mx-auto pt-2 max-w-md">
      <div class="text-center">
        ...
        <p class="text-sm mt-2">
          If the issue persists, please follow the "Configure a Connected App for the OAuth 2.0 Client Credentials Flow"
          instructions on the
          <a
  ...
  defp handle_code_update(code, socket) do
    ...
    {:ok, client} = Salesforce.get_token(client, code: code)
  ...
  defp get_userinfo(pid, socket) do
    ...
    Salesforce.get_userinfo(client, token)
    |> case do
      {:ok, resp} ->
  ...
  defp refresh_token(pid, socket) do
    ...
    Salesforce.refresh_token(client, token)
    |> case do
      {:ok, token} ->
