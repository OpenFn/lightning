defmodule LightningWeb.WorkflowLive.AiAssistantComponent do
        ...
        <%= for message <- @session.messages do %>
          ...
          >
            ...
            <div class="break-words max-w-[80%]">
              <.formatted_content id={message.id} content={message.content} />
              <div
                :if={
                  !@pending_message.loading &&
                    last_assistant_message?(@session.messages, message)
                }
                class="flex justify-end mt-4"
              >
                <.link
                  href="#"
                  class="flex items-center gap-1 text-xs text-gray-500 hover:underline hover:text-gray-700"
                  phx-click={JS.show(to: "#ai-assistant-disclaimer")}
                >
                  <.icon
                    name="hero-information-circle"
                    class="h-5 w-5 text-gray-500"
                  /> This response is generated by an algorithm.
                </.link>
              </div>
            </div>
          </div>
  ...
  def formatted_content(assigns) do
    ...
    ~H"""
    <article id={"codeblock-#{@id}"} phx-hook="SyntaxHighlighter">
      <%= @content %>
    </article>
    """
  end
  ...
  defp last_assistant_message?(messages, current_message) do
    messages
    |> Enum.filter(&(&1.role == :assistant))
    |> List.last()
    |> then(&(&1.id == current_message.id))
  end
