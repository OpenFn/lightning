defmodule Lightning.ExportUtils do
  ...
  defp job_to_treenode(job) do
    ...
    {parent_id, trigger} = case trigger.type do
      :webhook -> {nil, "webhook"}
      :cron -> {nil, %{cron: trigger.cron} }
      :on_job_success -> {parent_job.id, %{"on-success": parent_job.name |> String.replace(" ", "-")} }
      :on_job_failure -> {parent_job.id, %{"on-fail": parent_job.name |> String.replace(" ", "-")} }
    end
  ...
  defp to_new_yaml(map, indentation \\ "") do
    ...
    |> Enum.map(fn {key, value} ->
      cond do
        is_bitstring(value) -> "#{indentation}#{handle_bitstring(key, value, indentation)}"
        is_number(value) -> "#{indentation}#{key}: #{value}"
        is_boolean(value) -> "#{indentation}#{key}: #{Atom.to_string(value)}"
        value == %{} -> "#{indentation}#{key}:"
        is_map(value) -> "#{indentation}#{key}:\n#{to_new_yaml(value, "#{indentation}  ")}"
        value == [] -> "#{indentation}#{key}:"
        is_list(value) -> "#{indentation}#{key}:\n#{Enum.map(value, fn map ->
          cond do
  ...
  def build_yaml_tree(workflows, project) do
    ...
    workflows_map = Enum.reduce(workflows, %{}, fn workflow, acc ->
