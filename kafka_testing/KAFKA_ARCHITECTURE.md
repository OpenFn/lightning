...
from the Kafka clster. The pipleine also creates a
`Lightning.KafkaTriggers.TriggerKafkaMessageRecord` entry for each message
received. The `TriggerKafkaMessageRecord` entry is to identify and discard
duplicate messages.
...
The pipeline creates a Workflow, Dataclip and Run for each messages recevied. It
...
Should the pipeline fail due to persistence errors, the
pipeline offset timestamp will be reset to before the timestamp of the
failed message. This means that there is a risk of duplicate messages being
requested from the Kafka cluster but this should be mitigated by the
`TriggerKafkaMessageRecord` entries (assuming they have been retained - see the
notes on `Lightning.KafkaTriggers.DuplicateTrackingCleanupWorker`).
...
was provided in `initial_offset_reset_policy`. This feature was added so that
Lightning users will not face potential billing consequences due to reprocessing
a large number of messages when a trigger is re-enabled.
