...
| `SENTRY_FRONTEND_DSN`                             | DSN for browser-side (React/JS) error reporting. Exposed to every visitor's browser, so use a separate Sentry project from `SENTRY_DSN`. Unset disables client-side reporting.                                                                                  |
