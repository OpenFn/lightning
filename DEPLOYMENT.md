...
| `K8S_CROSS_CLUSTER_DISCOVERY_ENABLED`     | If this is set, Lightning will add an additional Erlang node discovery topology to the current set. The new topology uses the existing Postgresql connection to do so. It is intended to allow node discovery when nodes are resident on multiple k8s clusters.                                                                                                                                                                                     |
