defmodule AgentRegistry do
  ...
  def handle_call({:create, pid}, _from, {pids, refs}) do
    Process.get(:"$ancestors") |> IO.inspect()
...
AgentRegistry.get(:a) |> IO.inspect(label: "get from main")

task =
  Task.async(fn ->
    Process.get(:"$ancestors") |> IO.inspect()
    Process.sleep(100)
    AgentRegistry.get(:a) |> IO.inspect(label: "get from task")
  end)

AgentRegistry.register(task.pid)

AgentRegistry.put(:a, 1)
