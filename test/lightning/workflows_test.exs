defmodule Lightning.WorkflowsTest do
  ...
  describe "soft delete jobs" do
    test "schedule_job_deletion/1 schedules a deletion date for a given job according to the :purge_deleted_after_days env" do
      days = Application.get_env(:lightning, :purge_deleted_after_days)

      %{jobs: [job], edges: [edge]} = insert(:simple_workflow)

      refute job.scheduled_deletion
      refute edge.scheduled_deletion

      {:ok, %{edge: updated_edge, job: updated_job}} =
        Workflows.schedule_job_deletion(job)

      assert updated_edge.id === edge.id
      assert updated_edge.scheduled_deletion

      assert updated_job.id === job.id
      assert updated_job.scheduled_deletion

      now = DateTime.utc_now() |> DateTime.truncate(:second)

      assert updated_edge.scheduled_deletion |> Timex.diff(now, :days) ==
               days

      assert updated_job.scheduled_deletion |> Timex.diff(now, :days) ==
               days
    end

    test "get_edge_by_target_job/1 returns the edge leading to a given target job in a workflow" do
      workflow = insert(:workflow)
      trigger = insert(:trigger, workflow: workflow)

      [first_job, second_job] =
        insert_list(2, :job, workflow: workflow)

      first_edge =
        insert(:edge,
          workflow: workflow,
          source_trigger_id: trigger.id,
          target_job_id: first_job.id
        )
      ...
      second_edge =
        insert(:edge,
          ...
          target_job_id: second_job.id
        )

      edge = Workflows.get_edge_by_target_job(first_job)

      assert edge.id === first_edge.id
      assert edge.id !== second_edge.id
    end
  end
end
