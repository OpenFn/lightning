defmodule Lightning.AiAssistantTest do
  ...
  describe "extract_code_from_markdown/1" do
    test "extracts first javascript code block" do
      markdown = """
      Here's your code:
      ...
      assert AiAssistant.extract_code_from_markdown(markdown) ==
               "fn(state => state);"
    end

    test "extracts js code block (short syntax)" do
      markdown = """
      ```js
      console.log('hello');
      ```
    ...
    test "extracts first JS block when multiple languages present" do
      markdown = """
      ```python
      print('hello')
      ```
      ...
      # Should extract first JS block, not Python
      assert AiAssistant.extract_code_from_markdown(markdown) ==
               "console.log('world');"
    end

    test "returns nil for markdown with no code blocks" do
      assert AiAssistant.extract_code_from_markdown("Just text") == nil
    end

    test "returns nil for nil input" do
      assert AiAssistant.extract_code_from_markdown(nil) == nil
    end
    ...
    test "handles multi-line code blocks" do
      ...
      assert AiAssistant.extract_code_from_markdown(markdown) == expected
    end

    test "returns first code block when multiple JS blocks exist" do
      markdown = """
      ```js
      const first = 'first';
      ```
