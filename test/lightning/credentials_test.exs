defmodule Lightning.CredentialsTest do
  ...
  describe "maybe_refresh_token/1" do
    ...
    test "refreshes oauth credentials when they are about to expire" do
      ...
      oauth_client_attrs = %{
        client_id: "clientID_hopscotch_unicorns",
        client_secret: "topSecret_shh_dontTellAnyone",
      ...
      oauth_client =
        insert(:oauth_client,
          client_id: oauth_client_attrs.client_id,
          client_secret: oauth_client_attrs.client_secret,
          token_endpoint: oauth_client_attrs.token_endpoint
        )
      ...
      expect(Lightning.AuthProviders.OauthHTTPClient.Mock, :call, fn
        env, _opts
        when env.method == :post and env.url == oauth_client.token_endpoint ->
          assert env.body ==
                   %{
                     client_id: oauth_client.client_id,
                     client_secret: oauth_client.client_secret,
                     refresh_token: rotten_credential.body["refresh_token"],
                     grant_type: "refresh_token"
                   }
                   |> URI.encode_query()

          {:ok, %Tesla.Env{status: 200, body: Jason.encode!(fresh_token)}}
      end)
      ...
      # Assertions to verify that the credentials were indeed refreshed
      refute fresh_credential == rotten_credential,
             "Expected credentials to be refreshed"

      assert rotten_credential.body == rotten_token
      assert fresh_credential.body == fresh_token
      ...
      assert fresh_credential.body["expires_at"] >
             ...
             "Expected new expiry to be greater than the old expiry"
    end

    test "doesn't refreshe oauth credentials when they're oauth client is nil" do
      rotten_token = %{
        "access_token" =>
          "00DWS000000fDCb!AQEAQHRBgVJ4Bbb4XTr218Sn672cpnALW9FMkaATpdh8EwLhEJiNUrHg0eHiCBqHOh06F3pIyJym5gx4YD1vFv2fomMOOdzF",
        "apiVersion" => "",
