defmodule Lightning.Accounts.UserTest do
  ...
  describe "validate_project_access/2" do
    test "returns changeset when email is nil" do
      changeset = User.changeset(%User{}, %{})
      credential = insert(:credential)

      result = User.validate_project_access(changeset, credential)
      assert result == changeset
    end

    test "returns changeset when user doesn't exist" do
      changeset = User.changeset(%User{}, %{email: "nonexistent@example.com"})
      credential = insert(:credential)
    ...
    test "returns changeset when user has access to all projects" do
      user = insert(:user)
    ...
    test "adds error when user lacks access to projects" do
      user = insert(:user)
    ...
    test "adds error with multiple project names when user lacks access to multiple projects" do
      user = insert(:user)
      project1 = insert(:project, name: "Project A")
      project2 = insert(:project, name: "Project B")
      ...
      {_, {error_message, _}} =
        Enum.find(result.errors, fn {field, _} -> field == :email end)

      "User doesn't have access to these projects: " <> projects_str =
        error_message
  ...
  describe "validate_not_same_user/4" do
    test "returns changeset when field value is nil" do
      current_user = %User{email: "current@example.com"}
      changeset = User.changeset(%User{}, %{})

      result = User.validate_not_same_user(changeset, :email, current_user)
      assert result == changeset
    end

    test "returns changeset when values are different" do
      current_user = %User{email: "current@example.com"}
      changeset = User.changeset(%User{}, %{email: "different@example.com"})

      result = User.validate_not_same_user(changeset, :email, current_user)
      assert result == changeset
    end

    test "adds error when values are the same" do
      current_user = %User{email: "same@example.com"}
      changeset = User.changeset(%User{}, %{email: "same@example.com"})

      result = User.validate_not_same_user(changeset, :email, current_user)

      assert {:email, {"Cannot be the same as current user", []}} in result.errors
    end

    test "uses custom error message when provided" do
      current_user = %User{email: "same@example.com"}
      changeset = User.changeset(%User{}, %{email: "same@example.com"})

      result =
        User.validate_not_same_user(changeset, :email, current_user,
          message: "Custom error"
        )

      assert {:email, {"Custom error", []}} in result.errors
    end
  end

  describe "validate_email_not_exists/1" do
    test "returns no error when email exists" do
      user = insert(:user)
      ...
      result = User.validate_email_not_exists(changeset)
      assert result.errors == []
    end

    test "adds error when email does not exist" do
      changeset =
        %User{}
        |> Ecto.Changeset.cast(%{email: "nonexistent@example.com"}, [:email])

      result = User.validate_email_not_exists(changeset)
      assert {:email, {"user does not exist", []}} in result.errors
    end

    test "returns changeset when no email is provided" do
      changeset =
        %User{}
