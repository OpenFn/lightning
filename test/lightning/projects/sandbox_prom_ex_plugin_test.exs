defmodule Lightning.Projects.SandboxPromExPluginTest do
  ...
  describe "seed_event_metrics/0" do
    test "seeds all metrics with initial zero values" do
      sandbox_created = [:lightning, :sandbox, :created]
      sandbox_merged = [:lightning, :sandbox, :merged]
      sandbox_deleted = [:lightning, :sandbox, :deleted]
      workflow_saved = [:lightning, :workflow, :saved]
      provisioner_import = [:lightning, :provisioner, :import]
      ...
      SandboxPromExPlugin.seed_event_metrics()

      # Sandbox lifecycle events seeded with count: 0
      assert_received {
        ^sandbox_created,
        ^ref,
        %{count: 0},
      ...
      # Workflow saved seeded for both sandbox and non-sandbox
      assert_received {
        ^workflow_saved,
        ^ref,
        %{count: 0},
      ...
      # Provisioner import seeded for both sandbox and non-sandbox
      assert_received {
        ^provisioner_import,
        ^ref,
        %{count: 0},
