defmodule Lightning.InstallAdaptorIconsTest do
  ...
  setup do
    tmp_dir =
      Path.join(
        System.tmp_dir!(),
        "install_adaptor_icons_test_#{System.unique_integer([:positive])}"
      )
  ...
  @tag :capture_log
  test "generates http adaptor icons correctly", %{icons_path: icons_path} do
    Mox.expect(Lightning.Tesla.Mock, :call, fn env, _opts ->
      assert env.url == @adaptors_tar_url
      {:ok, %Tesla.Env{status: 200, body: File.read!(@http_tar_path)}}
    end)
  ...
  test "generates dhis2 adaptor icons correctly", %{icons_path: icons_path} do
    Mox.expect(Lightning.Tesla.Mock, :call, fn env, _opts ->
      assert env.url == @adaptors_tar_url
      {:ok, %Tesla.Env{status: 200, body: File.read!(@dhis2_tar_path)}}
    end)
  ...
  @tag :capture_log
  test "generates both dhis2 and http adaptor icons correctly", %{
    icons_path: icons_path
  } do
    Mox.expect(Lightning.Tesla.Mock, :call, fn env, _opts ->
      assert env.url == @adaptors_tar_url
      {:ok, %Tesla.Env{status: 200, body: File.read!(@http_dhis2_tar_path)}}
    end)
