defmodule LightningWeb.PlugConfigsTest do
  use LightningWeb.ConnCase, async: true

  alias LightningWeb.PlugConfigs

  describe "decompress_body/2" do
    test "decompresses gzipped request body", %{conn: conn} do
      ...
      {:ok, decompressed_body, _conn} = PlugConfigs.decompress_body(conn, [])
    ...
    test "passes through non-gzipped request body", %{conn: conn} do
      ...
      # Call the decompress function (should just read normally)
      {:ok, body, _conn} = PlugConfigs.decompress_body(conn, [])
