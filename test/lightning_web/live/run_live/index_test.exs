defmodule LightningWeb.RunLive.IndexTest do
  ...
  describe "filter chips" do
    ...
    # Summer DST boundary for Europe/London (UTC+1):
    # local 10:00..10:10 must map to 09:00Z..09:10Z.
    test "received date filter applies summer DST offset for Europe/London", %{
      conn: conn,
      project: project,
      workflow: workflow,
      jobs: jobs
      ...
      matching_utc = ~U[2026-07-15 09:05:00Z]
      non_matching_utc = ~U[2026-07-15 10:05:00Z]
      ...
      {:ok, view, _html} =
        live_async(
          ...
          Routes.project_run_index_path(conn, :index, project.id,
            filters: %{
              wo_date_after: "2026-07-15T10:00",
              wo_date_before: "2026-07-15T10:10"
            }
          )
        )
