defmodule LightningWeb.WorkflowLive.CollaborateTest do
  ...
  describe "build_from_scratch event" do
    test "creates a webhook workflow and navigates to it with the trigger param",
         %{conn: conn} do
      user = insert(:user)
      ...
      new_workflow =
        Lightning.Workflows.get_workflow!(new_workflow_id)
        |> Lightning.Repo.preload(:triggers)

      assert new_workflow.project_id == project.id
      assert [%{id: ^trigger_id, type: :webhook}] = new_workflow.triggers
    end

    test "user without project access gets a flash error", %{conn: conn} do
      user = insert(:user)
      ...
      # Revoke the user's membership after mount (e.g. removed from the
      # project mid-session) so the LiveView's cached `project`/`current_user`
      # assigns remain, but Bodyguard's live authorization check now fails.
      ...
      assert %{"error" => "You don't have permission to create workflows"} =
               :sys.get_state(view.pid).socket.assigns.flash
    end

    test "viewer gets a flash error and no workflow is created", %{conn: conn} do
      user = insert(:user)
