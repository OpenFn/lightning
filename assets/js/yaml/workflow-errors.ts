...
export class SchemaValidationError extends WorkflowError {
  constructor(details: {
    ...
    let message: string;
    let errorDetails: Partial<WorkflowErrorDetails> = {
    ...
    switch (details.keyword) {
      ...
      default:
        code = WorkflowErrorCode.SCHEMA_INVALID_VALUE;
        message = details.message || `Validation error at ${details.instancePath}`;
    }
...
export function createWorkflowError(error: unknown): WorkflowError {
  ...
  if (error instanceof Error) {
    ...
    const jobRefMatch = message.match(/(?:SourceJob|TargetJob):\s*'([^']+)'\s*specified by edge\s*'([^']+)'/);
    if (jobRefMatch) {
      const isSource = message.includes('SourceJob:');
      return new JobNotFoundError(jobRefMatch[1], jobRefMatch[2], isSource);
    }
    
    const triggerRefMatch = message.match(/SourceTrigger:\s*'([^']+)'\s*specified by edge\s*'([^']+)'/);
    if (triggerRefMatch) {
      return new TriggerNotFoundError(triggerRefMatch[1], triggerRefMatch[2]);
    }
    
    // Check for duplicate job name
    const duplicateMatch = message.match(/Duplicate job name\s*'([^']+)'\s*found at\s*'jobs\/([^']+)'/);
    if (duplicateMatch) {
      return new DuplicateJobNameError(duplicateMatch[1], duplicateMatch[2]);
    }
    
    // Check for YAML syntax errors
    if (message.includes('YAML') || message.includes('expected') || message.includes('unexpected')) {
