  ...
  <>
    {data.label && data.name && (
      <>
        ...
        <pre className="inline text-xs align-bottom font-monospace ml-2">
          ({data.name})
        </pre>
      </>
    )}
    ...
    {data.type && (
      <span className="inline-block ml-4 mr-4 rounded-md border-secondary-300 text-slate-500 bg-sky-100/75 px-1 py-px cursor-default">
        {data.type}
      </span>
    )}
  </>
...
const Entity = ({ data, level }: EntityProps) => {
    ...
    return (
      <details>
        ...
        <ul className="list-disc ml-4">
          {mapChildren(data, e => (
            <Entity data={e} key={e.name} level={level + 1} />
          ))}
        </ul>
      </details>
    );
  ...
  if (typeof data === 'string') {
    return (
      <li className={`${summaryStyle} cursor-default ${indent}`}>"{data}"</li>
    );
  }
  // TODO how do we drive formatting rules for adaptor specific types?
  return (
    <li className={`${summaryStyle} cursor-default ${indent}`}>
      <Label data={data} />
    </li>
  );
};
