...
import { importComponent } from '#/react/lib/import-component';
import { isReactContainerElement } from '#/react/lib/is-react-container-element';
import { isReactHookedElement } from '#/react/lib/is-react-hooked-element';
import { lazyLoadComponent } from '#/react/lib/lazy-load-component';
import { replaceEqualDeep } from '#/react/lib/replace-equal-deep';
import { withProps } from '#/react/lib/with-props';
import { RootObserver } from '#/react/lib/root-observer';

import { Boundary } from '#/react/components/Boundary';
...
export const ReactComponent = {
  mounted() {
    ...
    console.debug(this._errorMsg('mounted'));
  ...
  updated() {
    console.debug(this._errorMsg('updated'));
    this._setProps();
  },

  beforeDestroy() {
    console.debug(this._errorMsg('beforeDestroy'));
    this._unmount();
  },

  destroyed() {
    console.debug(this._errorMsg('destroyed'));
  ...
  _subscribe(onPropsChange) {
    console.debug(this._errorMsg('_subscribe'));
    this._listeners.add(onPropsChange);

    const unsubscribe = () => {
      console.debug(this._errorMsg('_subscribe unsubscribe'));
  ...
  _getProps() {
    console.debug(this._errorMsg('_getProps'));
    invariant(
  ...
  addPortal(render, container, key) {
    console.debug(this._errorMsg('addPortal'));
    warning(
  ...
  removePortal(key) {
    console.debug(this._errorMsg('removePortal'));
    warning(
      this._portals.has(key),
  ...
  _getPortals() {
    console.debug(this._errorMsg('_getPortals'));
    return this._portals;
  },

  _onBoundary(element) {
    console.debug(this._errorMsg('_onBoundary'));
    this.__view.liveSocket.requestDOMUpdate(() => {
      console.debug(this._errorMsg('_onBoundary callback'));
  ...
  _setProps() {
    console.debug(this._errorMsg('_setProps'));
    invariant(
  ...
  _rerender() {
    console.debug(this._errorMsg('_rerender'));
  ...
  _mount() {
    console.debug(this._errorMsg('_mount'));
  ...
  _onBeforeUnmount(callback) {
    console.debug(this._errorMsg('_onBeforeUnmount'));
    this._beforeUnmountCallbacks.add(callback);
  },

  _beforeUnmount() {
    console.debug(this._errorMsg('_beforeUnmount'));
  ...
  _getKey() {
    console.debug(this._errorMsg('_getKey'));
  ...
  _mountRoot() {
    console.debug(this._errorMsg('_mountRoot'));
    invariant(
      this._root == null,
  ...
  _mountPortal() {
    console.debug(this._errorMsg('_mountPortal'));
    ...
    const unsubscribe = rootObserver.subscribe(this._portalTarget, message => {
      console.debug(
        this._errorMsg('_mountPortal receive message ' + message.type)
      );
      switch (message.type) {
        case 'mounted': {
  ...
  _unmount() {
    console.debug(this._errorMsg('_unmount'));
    invariant(
  ...
  _unmountRoot() {
    console.debug(this._errorMsg('_unmountRoot'));
    invariant(this._root != null, this._errorMsg('No React root to unmount!'));
    this._root.unmount();
  },

  _unmountPortal() {
    console.debug(this._errorMsg('_unmountPortal'));
    invariant(
