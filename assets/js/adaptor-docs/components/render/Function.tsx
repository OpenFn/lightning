...
const getSignature = (fn: FunctionDescription) => {
  ...
  return (
    <span>
      {fn.magic ? (
        <span style={{ float: 'left', marginLeft: '-18px' }}>✨</span>
      ) : (
        ''
      )}
      {[fn.name, '(', paramList.join(', '), ')'].join('')}
    </span>
  );
};
      ...
      <div style={{ marginTop: '-6px' }}>
        <div className="w-full px-5 text-right" style={{ height: '13px' }}>
          <PreButton
            label="COPY"
            onClick={() => doCopy(code)}
            tooltip="Copy this example to the clipboard"
          />
          {onInsert && (
            <PreButton
              label="ADD"
              onClick={() => onInsert(code)}
              tooltip="Add this snippet to the end of the code"
            />
          )}
        </div>
