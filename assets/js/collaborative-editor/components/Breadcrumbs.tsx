...
export function BreadcrumbText({
    ...
    <>
      ...
      <span
        className={cn(
          ...
          icon ? 'ml-2' : '',
          onClick && 'group-hover:text-gray-700'
        )}
      >
        {children}
      </span>
  ...
  // When a click handler is provided, render as a button so the title can act
  // as a navigation control (e.g. returning to the root workflow editor view).
  if (onClick) {
    return (
      <button
        type="button"
