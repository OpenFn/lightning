...
export function useVersionMismatch(
  ...
  return useMemo(() => {
    ...
    // 1. Viewing latest version (not a specific snapshot)
    // 2. Run was executed on a different version
    const isViewingLatest = workflowLockVersion === latestSnapshotLockVersion;
    const runUsedDifferentVersion =
      selectedWorkOrder.version !== workflowLockVersion;

    if (isViewingLatest && runUsedDifferentVersion) {
      return {
