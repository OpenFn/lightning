...
test('can navigate to collaborative editor and see React component', async ({ page, }) => {
    ...
    await page.evaluate(([workflowId]) => {
        window.liveSocket.historyRedirect({ isTrusted: true }, `/projects/4adf2644-ed4e-4f97-a24c-ab35b3cb1efa/w/${workflowId}/collaborate`, 'push', null, document.querySelector(`#workflow-${workflowId}`));
    }, [workflowId]);
    await page.waitForLoadState('networkidle');
