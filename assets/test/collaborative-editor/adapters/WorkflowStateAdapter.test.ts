/**
 * Tests for WorkflowStateAdapter
 *
...
import { WorkflowStateAdapter } from '../../../js/collaborative-editor/adapters/WorkflowStateAdapter';
...
describe('WorkflowStateAdapter', () => {
  ...
  describe('Job transformations', () => {
    ...
    test('handles multiline body', () => {
      ...
      expect(bodyText.toString()).toBe('fn(state => {\n  console.log(state);\n  return state;\n});');
    });
  });
  ...
  describe('applyToYDoc full workflow', () => {
    ...
    test('clears existing Y.Doc data before import', () => {
      ...
      const initialWorkflow: WorkflowState = {
        id: 'initial',
        name: 'Initial',
        jobs: [{ id: 'old-job', name: 'Old', adaptor: '@openfn/language-common@latest', body: 'old()' }],
        triggers: [],
        edges: [],
        positions: null,
      };

      WorkflowStateAdapter.applyToYDoc(ydoc, initialWorkflow);
      ...
      const newWorkflow: WorkflowState = {
        id: 'new',
        name: 'New',
        jobs: [{ id: 'new-job', name: 'New', adaptor: '@openfn/language-common@latest', body: 'new()' }],
        triggers: [],
        edges: [],
        positions: null,
      };

      WorkflowStateAdapter.applyToYDoc(ydoc, newWorkflow);
    ...
    test('applies all transformations in single transaction', () => {
      const workflowState: WorkflowState = {
        ...
        name: 'Atomic Test',
        jobs: [
          { id: 'job-1', name: 'Job 1', adaptor: '@openfn/language-common@latest', body: 'fn1()' },
          { id: 'job-2', name: 'Job 2', adaptor: '@openfn/language-common@latest', body: 'fn2()' },
        ],
        triggers: [
          { id: 'trigger-1', type: 'webhook', enabled: true },
        ],
        edges: [
          { id: 'edge-1', source_trigger_id: 'trigger-1', target_job_id: 'job-1', condition_type: 'always', enabled: true },
          { id: 'edge-2', source_job_id: 'job-1', target_job_id: 'job-2', condition_type: 'on_job_success', enabled: true },
        ],
        positions: {
          'trigger-1': { x: 0, y: 0 },
          'job-1': { x: 200, y: 0 },
